...
def aggregate_team_stats(stats_df: pd.DataFrame) -> pd.DataFrame:
    team_stats = (
        stats_df.groupby(["team_abbreviation", "game_week"])
        .agg(
    ...
    team_stats["completion_pct"] = team_stats["pass_completions"] / team_stats[
        "pass_attempts"
    ].replace(0, np.nan)
    team_stats["yards_per_rush"] = team_stats["rush_yards"] / team_stats[
        "rush_attempts"
    ].replace(0, np.nan)
    ...
    team_stats["defensive_stops"] = (
        team_stats["sacks"] + team_stats["interceptions"]
    )
    return team_stats
...
def merge_with_lag(stats, sentiment, lag: int) -> pd.DataFrame:
    ...
    return stats.merge(
        s, on=["team_abbreviation", "game_week"], how="inner"
    ).assign(lag=lag)
...
def run_block(stats_path: Path, out_csv: str) -> None:
    stats_df = pd.read_csv(stats_path.expanduser(), encoding="utf-8")
    ...
    for label, dfb in [
        ("concurrent", google_concurrent),
        ("sentiment leads (+1)", google_sent_leads),
        ("perf leads (-1)", google_perf_leads),
    ]:
        print(
            f"   {label}: {len(dfb)} team-week pairs | "
            f"{dfb['team_abbreviation'].nunique()} teams"
        )

    google_corr_concurrent = compute_correlations(
        google_concurrent, "concurrent (W vs W)"
    )
    google_corr_sent_leads = compute_correlations(
        google_sent_leads, "sentiment leads (sent W -> perf W+1)"
    )
    google_corr_perf_leads = compute_correlations(
        google_perf_leads, "perf leads (perf W -> sent W+1)"
    )
    ...
    google_all_corrs.to_csv(out_dir / out_csv, index=False, encoding="utf-8")
    print(f"\nSaved {out_csv}")
