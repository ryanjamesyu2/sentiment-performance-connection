...
def build_name_map(twitter_names, stats_names, score_cutoff=85):
    ...
    if low_confidence:
        print(f"\n⚠️  {len(low_confidence)} low-confidence matches — review manually:")
...
POSITION_CONFIG = {
    "QB": {
        "positions": ["QB"],
        "metrics": [
            "stats_passing_pass_yards", "stats_passing_pass_td",
            "stats_passing_pass_int", "stats_passing_qb_rating",
            "stats_passing_pass_attempts", "stats_passing_pass_completions",
            "stats_rushing_rush_yards", "stats_rushing_rush_td",
            "fantasy_points_ppr", "stats_snap_counts_offense_snaps",
        ],
    },
    "RB": {
        "positions": ["RB", "FB"],
...
LAG_FRAMES = {
    ...
    "sentiment leads (sent W → perf W+1)": sent_leads_pdf,
...
per_player_df = (
    pd.DataFrame(per_player_results)
    .sort_values("spearman_r", key=abs, ascending=False)
)
...
print("=" * 70)
DISP_P = ["player", "team", "position", "n", "pearson_r", "pearson_sig",
